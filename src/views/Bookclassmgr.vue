<template>
    <el-main style="padding:0px">
      ...
      <el-main>
        <el-button type="primary" @click="dialogVisible = true" plain>新增分类</el-button>
        <el-table v-loading="loading" :data="tableData.slice((currentPage - 1) * pagesize, currentPage * pagesize)" style="width: 100%">
          ...
          <el-table-column :show-overflow-tooltip="true" prop="bookClass" label="分类名" min-width="100px"></el-table-column>
        ...
        <el-dialog
          ...
          :before-close="handleClose">
            <el-form ref="form" status-icon :model="form" :rules="rules" label-width="80px">
              <el-form-item label="分类名" prop="bookClass">
                <el-input v-model="form.bookClass"></el-input>
              </el-form-item>
            </el-form>
        ...
        <el-dialog
          ...
          :before-close="handleClose_edit">
            <el-form ref="edit_form" status-icon :model="edit_digital_array" :rules="rules" label-width="80px">
              <el-form-item label="分类名" prop="bookClass">
                <el-input v-model="edit_digital_array.bookClass"></el-input>
              </el-form-item>
            </el-form>
          <span slot="footer" class="dialog-footer">
        ...
        methods: {
          ...
          handleDel(formdata) {
            this.$confirm('此操作将永久删除该分类, 是否继续?', '提示', {
              ...
              axios.get('/userApi/delBookClass',{params:{id:formdata.id}, headers: {'token': localStorage.getItem('ljc_token')}})
                .then(res => {
                  if(res.data.code != null && res.data.code == 400) {
                    if (res.data.code != null && res.data.code == 400) {
                        this.$message.error('登录状态过期，请重新登录');
                  ...
                  if(res.data.code == 200) {
                    this.$message({
                      type: 'success',
                      message: '删除分类：“'+formdata.bookClass+'”成功!'
                    });
                    for(var i=0;i<this.tableData.length;i++) {
